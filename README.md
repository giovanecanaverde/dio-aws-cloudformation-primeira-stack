...
# Passos Realizados:
 Modelagem: Criação do arquivo `template.yaml` definindo uma instância EC2 (`t2.micro`).
 Validação: Verificação da sintaxe do arquivo para garantir que a estrutura declarada respeita os padrões da AWS.
 Provisionamento: Upload do template no console do CloudFormation para a criação e inicialização automatizada da Stack.
